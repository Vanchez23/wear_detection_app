...
from wearing_detector.utils.general import check_anchor_order, make_divisible, check_file, set_logging
from wearing_detector.utils.torch_utils import (
...
class Detect(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            x[i] = x[i].view(
                bs, self.na, self.no, ny, nx).permute(
...
class Model(nn.Module):
    ...
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None):
        ...
        if nc and nc != self.yaml['nc']:
            print(
                'Overriding model.yaml nc=%g with nc=%g' %
                (self.yaml['nc'], nc))
    ...
    def forward(self, x, augment=False, profile=False):
        if augment:
            ...
            for si, fi in zip(s, f):
                ...
                # xi[0].numpy().transpose((1, 2, 0))[:, :, ::-1])  # save
                yi[..., :4] /= si  # de-scale
                if fi == 2:
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(
        ...
        if m in [Conv, Bottleneck, SPP, DWConv, MixConv2d,
