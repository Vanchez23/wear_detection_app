...
def intersect_dicts(da, db, exclude=()):
    ...
    return {k: v for k, v in da.items() if k in db and not any(
        x in k for x in exclude) and v.shape == db[k].shape}
...
def find_modules(model, mclass=nn.Conv2d):
    ...
    return [i for i, m in enumerate(
        model.module_list) if isinstance(m, mclass)]
